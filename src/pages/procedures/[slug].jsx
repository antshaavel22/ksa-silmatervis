...
import { generateMedicalProcedureSchema, generateOrganizationSchema, combineSchemas } from '@/lib/schema';
import { BRAND_COLORS } from '@/lib/constants';
import pilotPages from '@/public/data/pilot-pages.json';

export default function ProcedurePage({ page, relatedPages }) {
  ...
  const procedureSchema = generateMedicalProcedureSchema(page);
  const orgSchema = generateOrganizationSchema();
  const combinedSchemas = combineSchemas([procedureSchema, orgSchema]);
  ...
  return (
    <Layout>
      <Head>
        ...
        />
        <meta name="keywords" content={`${page.title}, procedure, surgery, eye care, KSA`} />
        <meta property="og:title" content={`${page.title} - Silmatervis`} />
        <meta property="og:description" content={page.description} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://silmatervis.ksa.ee/procedures/${page.slug}`} />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {page.treatments && page.treatments.length > 0 && (
            <div className="mb-12">
              ...
              <ul className="space-y-3">
                {page.treatments.map((outcome, idx) => (
                  <li key={idx} className="flex gap-3 items-start">
                    <span
                      className="w-2 h-2 rounded-full mt-2 flex-shrink-0"
                      style={{ backgroundColor: BRAND_COLORS.primary }}
          ...
          >
            ...
            <p className="text-gray-700 mb-6">
              KSA Vision Clinic performs this procedure with world-class expertise.
              Schedule a consultation with our ophthalmologists to discuss if this
...
export async function getStaticProps({ params }) {
  const page = pilotPages.find(
    (p) => p.slug === params.slug && p.category === 'procedures'
  );
...
export async function getStaticPaths() {
  const paths = pilotPages
    .filter((p) => p.category === 'procedures')
    .map((page) => ({
